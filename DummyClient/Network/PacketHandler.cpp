...
{
    static ServerPacketHandlerMap instance;
    return instance;
}
    ...
    gRoom->MakeJob([session, id = payload.id()]
                   {
                       ...
                       std::static_pointer_cast<ServerSession>(session)->SetPlayer(player);
                       ...
                       session->SendAsync(PacketUtils::MakeSendBuffer(payload, PacketId::ChatNotify));*/
                   });
